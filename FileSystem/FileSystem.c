...
void fileSystemCrear() {
	...
	grasaFS->pBitmap = bitarray_create(
			(char*)grasaFS->disco->mem + (blkBitmap * BLOCK_SIZE), tamanioBitmap);
...
static int grasa_readdir(const char *path, void *buf, fuse_fill_dir_t filler, off_t offset, struct fuse_file_info *fi) {
    ...
    if (strcmp(path, DIRECTORIO_RAIZ) == 0){
    	for (i = 0; i<1024; i++){
    		if (grasaFS->nodos[i].parent_dir_block == 0 && grasaFS->nodos[i].state != BORRADO){
	...
	for (i = 0; i<1024; i++){
				if (grasaFS->nodos[i].parent_dir_block == blkDirPadre && grasaFS->nodos[i].state != BORRADO){
					filler(buf, (char*)grasaFS->nodos[i].filename, NULL, 0);
				}
			}
    }
...
//    		pthread_mutex_lock(&mutex);
    		ptrGBloque *blk_direct = (ptrGBloque*) (grasaFS->disco->mem + grasaFS->nodos[blkDir].blk_indirect[blkIndirecto] * BLOCK_SIZE); //blk_direct = Array[1024]
...
//    					pthread_mutex_unlock(&mutex);
    					...
    					if (blkDirecto == 1024)
    					{
    						...
    						blkIndirecto++;
//    						pthread_mutex_lock(&mutex);
    						blk_direct =  (ptrGBloque*)(grasaFS->disco->mem + grasaFS->nodos[blkDir].blk_indirect[blkIndirecto] * BLOCK_SIZE);
//    						pthread_mutex_unlock(&mutex);
    					}
    				}
